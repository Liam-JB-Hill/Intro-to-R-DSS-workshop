...
1. First, we need to put some data in our project folder that we can then load into our R Environment. To do this, download the `r colourise("*ExampleSDQ_data.csv*","blue")`  dataset from [here](link to be added), and place it in the *my_first_R_project** directory that you created earlier (e.g. I created my project on my desktop, so I just dropped the file in its 'folder' there). If you’ve done this correctly the file should now be listed in R Studio in the bottom left pane (as per the screenshot below (added dataset circled in `r colourise("red","red")`):
